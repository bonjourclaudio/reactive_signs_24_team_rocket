...
function setupAllBLobs() {
  ...
  for (let i = 0; i < blobCount; i++) {
    ...
    let randomRadius = random(poster.vh, poster.vh*3);
    ...
    for (let i = 0; i < blobs.length; i++) {
      ...
      */
    backgroundBlobs.push(new Blob(randomX, randomY, randomRadius, color(random(220,250)) ));
...
/*
 if (lastNumber != poster.getCounter() && morphProgress>=1) {
    morphProgress = 0;
 }
...
function initializeBlobs(points) {
  ...
  for (let pt of points) {
    ...
    blobs.push(new Blob(pt.x, pt.y,randomRadius, color(random(20,70))));
  }
